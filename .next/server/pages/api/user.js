...
const getUsers = async (req, res) => {
  try {
    ...
    const users = await _models_userModel__WEBPACK_IMPORTED_MODULE_1__[/* default */ "a"].find().select('-password');
    res.json({
      users
    });
  } catch (err) {
...
const uploadInfor = async (req, res) => {
  try {
    ...
    const newUser = await _models_userModel__WEBPACK_IMPORTED_MODULE_1__[/* default */ "a"].findOneAndUpdate({
      _id: result.id
    }, {
      name,
      avatar
